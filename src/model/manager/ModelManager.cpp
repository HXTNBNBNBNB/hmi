#include "model/manager/ModelManager.hpp"
#include <iostream>
#include <algorithm>
...
void ModelManager::loadDefaultModels() {
    std::cout << "=== ModelManager: loading default scene models ===" << std::endl;

    std::string truckModelPath = "/opt/models/smallstruck.glb";
    if (loadModel("truck", truckModelPath)) {
    ...
    if (loadModel("person", personModelPath)) {
    ...
    if (loadModel("otruck", otherTruckModelPath)) {
    ...
    if (loadModel("wtruck", otherWTruckModelPath)) {
    ...
    if (loadModel("cube", cubeModelPath)) {
